...
| 4 | Work schedule: today highlight, edit-hours popup, real times | B | DEV | PAUSED for usage-limit reset (Lead, ~1:50 AM). Backend landed at `e0fe782`: migration 050's `schedule_event_overrides` table (applied live earlier, migration file now committed matching what ran), unified occurrence resolver in `lib/tasks/schedule-cancellations.ts`, `work/actions.ts` rewritten (no more free-text `addScheduleEvent`). **`app/(app)/work/page.tsx` is currently BROKEN/uncommitted** — still imports the removed `addScheduleEvent`, left dirty intentionally per Lead. `components/work/work-schedule-week.tsx` also uncommitted (untested, not yet wired). Cancel-this-week verdict: NOT YET DETERMINED — blocked by the page.tsx breakage, was about to fix and test when paused. Next: finish page.tsx (remove DomainScheduleView usage, wire WorkScheduleWeek + a new WorkHoursEditorDialog), then get the Cancel-this-week verdict live. |
