...
| `125` | ULM lead (42335) | `alter type lesson_status add value 'adopted'` — **its own single-statement `--no-transaction` file.** Split out of `124` because R33's guard correctly refused it inside an atomic file: Postgres cannot add an enum value inside a transaction | allocated |
